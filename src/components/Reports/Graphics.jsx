...
export const MovementsChart = ({payments, bills, sells, salaries}) => {
    ...
    const movementsPerMonth = (arr, month) => {
        ...
        const result = dataArr.filter(x => x.period === month.id+'-'+initialYear).reduce((a,b) => a + b.amount, 0)
        return result
    }
    ...
    const options = {
        ...
        title: {
            display: true,
            text: `MOVIMIENTOS DEL ${initialYear}`
        }
    }

    useEffect(() => {
        totalMovements()
    }, [payments, bills, sells, salaries, currency])
...
export const ExpensesChart = ({bills, sells, salaries}) => {
    ...
    const handleSum = (data) => {
        let result = data.filter(x => x.currency === currency ).reduce((a,b) => {return a + b.amount}, 0)
        return result
    }
    ...
    const options = {
        ...
        title: {
            display: true,
            text: `TOTAL POR TIPO DE GASTOS (${currency})`
        }
    }

    useEffect(() => {
        handleTotal()
    },[bills, sells, salaries, currency])
...
export const TableLastPayments = ({payments, list}) => {

    const currentPeriod = (new Date().getMonth()+1)+'-'+new Date().getFullYear()
    const lastPeriod = (new Date().getMonth())+'-'+new Date().getFullYear()

    const lastsPay = payments.filter(x => x.period <= currentPeriod && x.period >= lastPeriod ).sort((a, b) => a.date < b.date ? 1 : -1)
...
export const TableLastExpenses = ({bills, sells, salaries}) => {
    ...
    const handleLastItem = () =>{
        ...
        dbTables.forEach(table => {
            let info = table.filter(x => x.period <= currentPeriod && x.period >= lastPeriod ).sort((a, b) => a.date < b.date ? 1 : -1)
            result.push(...info)
        })
        setLastPay(result)
    }
